        ...
        try:
            ...
            if self.flask_result.get('domain') == '' and self.flask_result.get('token') == '':
                if retry < 5:
                    ...
                    self.result['msg'] = f"domain | token 获取失败 {self.flask_result}"
                    return self.result
...
if __name__ == '__main__':
    ...
    def run(task, retry: int = 0):
        try:
            ...
            while data_list:
                ...
                # with concurrent.futures.ThreadPoolExecutor(max_workers=int(1)) as executor:
                #     futures = [executor.submit(juming().query_domain, f"{item}-{task}") for item in data_list[:5000]]
                    futures = [executor.submit(juming().query_domain, f"{item}-{task}") for item in data_list]
    ...
    def main():
        while True:
            try:
                t1 = Thread(target=run, args=('history',))
                t1.start()
                t1.join()
            except Exception as ex:
                print(ex)
