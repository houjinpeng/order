...
headers = {
    ...
    'Referer': 'http://x.juming.com:9696/p/9889689/',
...
def get_minute(time_str):
    day = 0
    if '天' in time_str:
        day = int(time_str.split('天')[0])
        time_str = time_str.split('天')[-1]
    ...
    try:
        ...
        pass
    try:
        date_time = datetime.datetime.strptime(time_str, '%S秒')
    except Exception as error:
        pass
    try:
        ...
        return 10000000
    # 剩余时间小于设置的时间
    sy_shijian = day * 60 * 24 + hour * 60 + minute + second / 60
...
class PlaceJob(QtCore.QThread):
    ...
    # 监控 一直出
    def jiankong_ym(self, value, shiji_price, chujia, key):
        while True:
            # 一直出价  直到超额
            is_sccess = self.check_and_place(value, shiji_price, chujia, key)
            if is_sccess == False:
                try:
                    palce_list.remove(key)
                except:
                    pass
                return
    ...
    def bid(self, id, chujia, domain):
        ...
        try:
            ...
            headers['Referer'] = f'http://x.juming.com:9696/p/{id}/'
            headers['Content-Length'] = str(len(data))
            headers['Cookie'] = cookie
            resp = self.request_headler(url, headers=headers, data=data, method='post')
            if json.loads(resp.text)['msg'] == '出价成功！':
                log.logger.debug(json.loads(resp.text))
                return True
            elif '竞价已经结束' in json.loads(resp.text)['msg']:
                return False
            elif '出价失败，请至少出价' in json.loads(resp.text)['msg']:
                log.logger.debug(json.loads(resp.text))
                chujia = re.findall('\d+', json.loads(resp.text)['msg'])[0]
                try:
                    if int(jingjia_dict[domain]['max_price']) >= int(chujia):
                        # return self.bid(id, chujia, domain)
                        return False
                    return False
                except Exception as error:
                    log.logger.error(error)
            elif '出价失败，您出的价格没有改变' in json.loads(resp.text)['msg']:
                return True
            else:
                if '登录' in json.loads(resp.text)['msg']:
                    logging.info('重新登陆')
    ...
    def check_and_place(self, value, shiji_price, chujia, key):
        ...
        is_succeed = self.bid(value['id'], value['chujia'], value['ym'])
        if is_succeed == True:
            # 更改状态
            row_count = self.tableWidget.rowCount()
            for row in range(row_count):
        ...
        while True:
            ...
            try:
                for key, value in jingjia_dict.items():
                    ...
                    # 获取剩余时间
                    sy_shijian = get_minute(value['sytime'])
                    ...
                    if int(value['max_price']) + int(d['max_yijia']) < shiji_price:
                        # log.logger.debug(f'超出最大设定价格 {int(value["max_price"]) + int(d["max_price"])}' )
                        continue

                    try:
                        # 获取固定和不固定的时间   判断价格合不合适
                        # if jingjia_dict[key].get('is_guding') == '固定':
                        if is_guding_dict[key] == '固定':
                            if int(d['guding1']) <= sy_shijian <= int(d['guding2']):
                                # if key not in palce_list:
                                #     palce_list.append(key)
                                    # threading.Thread(target=self.jiankong_ym,args=(value, shiji_price, chujia, key)).start()
                                self.check_and_place(value, shiji_price, chujia, key)

                        # elif jingjia_dict[key].get('is_guding') == '不固定':
                        elif is_guding_dict[key] == '不固定':
                            if d['is_waibu_chujia'] == '0':
                                continue
                ...
                time.sleep(float(d['refresh_time']))
            except Exception as e:
                print(e)
                time.sleep(10)
                pass
...
class InItTable(QtCore.QThread):
    ...
    def run(self):
        global jingjia_dict
        global is_guding_dict
        while True:
            url = f'http://x.juming.com:9696/user_main/jj_list?page=1&limit={limit}&myjj=1'
            ...
            for domain in all_domain:
                ...
                try:
                    max_price = doamin_dict[domain['ym']]['max_price']
                except Exception:
                    max_price = d['max_price']  # 预设价
                daili_price = int(ticheng.split('/')[0].strip())
                shifu_price = chujia - (chujia - daili_price) * 0.6
                domain['shifu_price'] = int(shifu_price) - domain['ewqian']
                try:
                    jingjia_dict[domain['ym']]['is_guding'] = '不固定' if '不固定' in domain['dl_ts'] else '固定'
                except Exception:
                    pass
                is_guding_dict[domain['ym']] = '不固定' if '不固定' in domain['dl_ts'] else '固定'
                if doamin_dict.get(domain['ym']) == None:
                    doamin_dict[domain['ym']] = {}
                doamin_dict[domain['ym']]['id'] = domain['id']
                domain['is_guding'] = '不固定' if '不固定' in domain['dl_ts'] else '固定'

                if d['yincang'] == '2':
                    # 如果 最大金额加100 还没shifu大
                    if int(max_price) + 100 < shifu_price:
                        continue

                    if shifu_price > int(max_price) and status != '领先':
                        i += 1
                        domain['iflx'] = '已停止，超出预算'
            ...
            for row in range(table_row):
                try:
                    ...
                    jingjia_dict[ym]['id'] = doamin_dict[ym]['id']
                    ...
                    try:
                        ...
                        doamin_dict[ym]['max_price'] = self.table.item(row, 9).text()
                    except Exception:
                        jingjia_dict[ym]['max_price'] = d['max_price']
                    jingjia_dict[ym]['chujia'] = self.table.item(row, 10).text()
                    jingjia_dict[ym]['row_index'] = row
                    # jingjia_dict[ym]['is_guding'] = domain['is_guding']
                except Exception:
                    continue
...
class Refresh(QtCore.QThread):
    ...
    def run(self):
        ...
        for domain in all_domain:
            ...
            daili_price = int(ticheng.split('/')[0].strip())
            shifu_price = chujia - (chujia - daili_price) * 0.6
            domain['shifu_price'] = int(shifu_price)
            if d['yincang'] == '2':
                # if 最大额度+100 >= 实际的金额
...
class Bidding(QWidget):
    def __init__(self):
        ...
        self.initUI()
        t = threading.Thread(target=self.check_login)
        t.start()
        # t.setDaemon(True)

        if d.get('is_waibu_chujia') == '2':
            self.is_waibu_chujia.setChecked(True)
    ...
    def initUI(self):
        ...
        self.is_waibu_chujia = QCheckBox()

        self.grid.addWidget(self.is_waibu_chujia, 1, 0)
        # 出价时间设置
        self.chujia_time_t = QLabel("外部不固定是否出价 （选中出价 否则不出）")
        self.grid.addWidget(self.chujia_time_t, 1, 1)

        # 溢价范围
        self.tongyi_yijia = QLabel("溢价范围/元：")
        self.grid.addWidget(self.tongyi_yijia, 2, 0)
        try:
            self.max_yijia = QLineEdit(d['max_yijia'])
        except Exception:
            d['max_yijia'] = '20'
            self.max_yijia = QLineEdit(d['max_yijia'])
        self.max_yijia.setPlaceholderText("输入整数")
        self.max_yijia.setValidator(intValidator)
        self.grid.addWidget(self.max_yijia, 2, 1)
        ...
        self.guding_time2 = QLineEdit(d['guding2'])
        ...
        self.refresh_time.setPlaceholderText("输入刷新时间")
        ...
        self.tableWidget.setHorizontalHeaderLabels(
            ['id', '域名', '备注', '领先价', '实付价', '外部/提成', '剩余时间', '结束时间', '状态', '预设价', '出价额', '操作', '实际需付'])  # 设置行表头字段
    ...
    def shou_refresh_table(self):
        self.Refresh = Refresh(self.tableWidget)
        self.Refresh.signal.connect(self.init_table)
        self.Refresh.start()
        self.thread_list.append(self.Refresh)
    ...
    def init_table(self, parm):
        ...
        ticheng = f"{domain['wbqian']} / {domain['tc_num']}"  # 代理
        remark = domain['bz']
        shiji_price = domain['qian'] - domain['tc_num'] - domain['ewqian']  # 实际金额
        chujia = domain['zdqian']  # 出价金额
        # max_price = d['max_price']  # 预设价
        try:
            # max_price = jingjia_dict[ym]['max_price']  # 预设价
            max_price = doamin_dict[ym]['max_price']  # 预设价
            if max_price == d['max_price']:
                max_price = doamin_dict[ym.lower()]['max_price']  # 预设价
        except Exception:
            max_price = d['max_price']
        guding = '不固定' if '不固定' in domain['dl_ts'] else '固定'
        shifu_price = domain['shifu_price']
        self.set_domain_table(data_row,
                              [id, ym, remark, lx_price, shiji_price, ticheng, sytime, endtime, status, max_price,
                               chujia, shifu_price, guding])

    # 设置复选框
    def set_is_waibu_chujia(self):
        if d['is_waibu_chujia'] == 2:
            self.is_waibu_chujia.setChecked(True)
    ...
    def set_setting(self, event):
        ...
        try:
            ...
            is_waibu_chujia = self.is_waibu_chujia.checkState()
            ...
            setting_d = {"user_id": user_id,
                         ...
                         'max_yijia': max_yijia,
                         'is_waibu_chujia': str(is_waibu_chujia)
                         }
    ...
    def set_yusuan(self):
        ...
        for ys in yd:
            ys_data = ys.split('|')
            try:
                doamin_dict[ys_data[0]]['max_price'] = ys_data[1].strip()
            except Exception as e:
                doamin_dict[ys_data[0]] = {'max_price': ys_data[1].strip()}

            for row in range(row_count):
                try:
                    if self.tableWidget.item(row, 1).text().lower() == ys_data[0].lower():
                        # print(row,ys_data[0],self.tableWidget.item(row, 1).text())
                        # 修改预算
                        # 修改最大金额
                        try:
                            if jingjia_dict.get(ys_data[0]) == None:
                                doamin_dict[self.tableWidget.item(row, 1).text()]['max_price'] = ys_data[1].strip()
                                jingjia_dict.get(self.tableWidget.item(row, 1).text())['max_price'] = ys_data[1].strip()
                            else:
                                doamin_dict[self.tableWidget.item(row, 1).text()]['max_price'] = ys_data[1].strip()
                                jingjia_dict[ys_data[0]]['max_price'] = ys_data[1].strip()
                        except Exception:
                            pass
    ...
    # 设置点击事件
    # 出价按钮
    def buttonLink(self, domain, domain_id):
        widget = QWidget()
        chujiaBtn = QPushButton(domain)
        chujiaBtn.clicked.connect(lambda: self.tiao_botton(domain_id))
        chujiaBtn.setStyleSheet(''' text-align : center;
                                                        background-color : NavajoWhite;
                                                        height : 30px;
                                                          border-style: outset;
    ...
    def tiao_botton(self, domain_id):
        webbrowser.open(f'http://x.juming.com:9696/p/{domain_id}/')

    # 设置table
    def set_domain_table(self, data_row, data_list):
        self.tableWidget.setCellWidget(data_row, 0, self.buttonLink(data_list[0], data_list[0]))
        ...
        #################################################################
        # 预设价
        max_price = self.tableWidget.item(data_row, 9)
        domain = self.tableWidget.item(data_row, 1)
        try:
            if max_price == None or domain == None or doamin_dict.get(domain.text()) == None:
                # 添加数据 预算
                new_item = QTableWidgetItem(str(data_list[9]))
                self.tableWidget.setItem(data_row, 9, new_item)
                new_item.setTextAlignment(Qt.AlignCenter | Qt.AlignBottom)

            elif doamin_dict[domain.text()]['max_price'] != max_price.text():
                # 添加数据 预算
                new_item = QTableWidgetItem(str(data_list[9]))
                self.tableWidget.setItem(data_row, 9, new_item)
                new_item.setTextAlignment(Qt.AlignCenter | Qt.AlignBottom)
        ...
        # 添加数据 结束时间
        new_item = QTableWidgetItem(str(data_list[11]))
        new_item.setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsEnabled)
        self.tableWidget.setItem(data_row, 12, new_item)
        new_item.setTextAlignment(Qt.AlignCenter | Qt.AlignBottom)
    ...
    def check_login(self):
        ...
        global is_live
        while is_live == True:
            url = 'http://x.juming.com:9696/user/#/qiang_jj'
