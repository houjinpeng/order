...
class SlideVerificationCode():
    ...
    def slide_verification(self, driver, slide_element, distance):
        ...
        if '验证成功' not  in driver.page_source and self.count > 0:
            driver.refresh()
            return False
        return True
        # 滑动失败的情况下，重试count次
        # if start_url == end_url and self.count > 0:
        #     print("第{}次验证失败，开启重试".format(self.count))
        #     self.count -= 1
        #     try:
        #         slide_element = driver.find_element_by_xpath('//*[@id="tcaptcha_drag_thumb"]')
