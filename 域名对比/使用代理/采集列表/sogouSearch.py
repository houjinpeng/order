...
time_num = eval(config['time_num'])
domain_list_tmp = []
ym_list = []
ym_file = './conf/ym.txt'
paichu_file = './conf/paichu.txt'
paichu_list = []
try:
...
class SoGou():
    ...
    def index(self):
        ...
        while not domain_list.empty():
            ...
            # url = f'https://www.sogou.com/web?query=site%3A{domain}&_ast={str(time.time())[:10]}&_asf=www.sogou.com&w=01059900&cid=&s_from=result_up'
...
def refresh_domain():
    while True:
        global domain_list_tmp
        domain_list_tmp = []
        Juming().index()
        time.sleep(time_num * 60 * 60)
